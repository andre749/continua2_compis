        ...
        for (const string& r : grammar.rules) {
            ...
            for (auto& alt : alternatives) {
                ...
                for (size_t i = 0; i < symbols.size(); i++) {
                    ...
                    int iter=symbols.size()-1;
                    bool epsilon = true;
                    string lastOne{};
                    while (epsilon and i>=0) {

                        lastOne=symbols[iter];
                        epsilon = false;
                        if(grammar.nonTerminals.count(lastOne)) {
                            for (auto f: first.firstSets[lastOne]) {
                                if (f == "''") {
                                    epsilon = true;
                                }
                            }
                            for(auto s: followSets[left]){
                                followSets[lastOne].insert(s);
                            }
                        }
